...
do
  ...
  then
    ...
    chown "$(id -u):$(id -g)" "/opt/tplink/EAPController/properties/${BASENAME}"
  fi
done
...
then
  ...
  chown -R "$(id -u):$(id -g)" /opt/tplink/EAPController/data/html
fi
...
then
  ...
  chown -R "$(id -u):$(id -g)" /opt/tplink/EAPController/data/pdf
fi
...
then
  ...
  chown "$(id -u):$(id -g)" /opt/tplink/EAPController/data/db
  echo "done"
fi
...
then
  ...
  chown "$(id -u):$(id -g)" "${KEYSTORE_DIR}/eap.keystore"
