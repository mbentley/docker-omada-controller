    ...
    case "${ARCH}" in
      amd64|arm64|"")
        ...
        then
          ...
          wget -q -O - https://www.mongodb.org/static/pgp/server-7.0.asc | gpg -o /etc/apt/keyrings/mongodb-server-7.0.gpg --dearmor

          # add repo
          echo "deb [ arch=${ARCH} signed-by=/etc/apt/keyrings/mongodb-server-7.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/7.0 multiverse" | tee /etc/apt/sources.list.d/mongodb-org-7.0.list
